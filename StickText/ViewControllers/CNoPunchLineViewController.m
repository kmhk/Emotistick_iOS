...
@interface CNoPunchLineViewController ()

@end

@implementation CNoPunchLineViewController

- (void) viewWillAppear:(BOOL)animated {
    [super viewWillAppear:animated];
    
    self.title = @"Without Punch Line";
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return [dbManger.m_noPunchSticks count];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSDictionary * dicStick = [dbManger.m_noPunchSticks objectAtIndex:indexPath.row];
